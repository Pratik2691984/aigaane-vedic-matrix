...
class AcousticEngine {
  ...
  public trigger(index: number, baseFreq: number): { name: string; cents: number; freq: number; ratio?: string } {
    this.init();
    ...
    osc1.stop(now + 2.5);
    osc2.stop(now + 2.5);
    return { name: SRUTI_LABELS[index], cents, freq, ratio: num + '/' + den };
  }

  public playPattern(pattern: string, baseFreq: number, bpm: number) {
    this.init();
    if (!this.ctx || !this.master) return;
    const beat = 60 / bpm;
    let t = this.ctx.currentTime;
    for (const ch of pattern) {
      const dur = ch === 'G' ? beat * 2 : beat;
      const osc = this.ctx.createOscillator();
      const g = this.ctx.createGain();
      osc.type = 'sine';
      osc.frequency.value = ch === 'G' ? baseFreq * 1.5 : baseFreq;
